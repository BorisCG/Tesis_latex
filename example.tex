...
In the appendix~\ref{ap:A} the algorithm and equations are showed. From (\ref{eq:ap_xpred}) and (\ref{eq:ap_velpred}), the predicted positions and velocities are:
...
Where $\mathbf{x}_j$, $\mathbf{v}_j$ are the positions and velocities at time $t_j$, the individual time for each particle. Note that the index $j$ runs for all particles. The acceleration $\mathbf{a}_j$ and jerk $\dot{\mathbf{a}}_j$ are computed by equations:
...
Where corrections of four order have been made. The advantage of this method is that is only needed to calculate the acceleration and jerk in each step.
