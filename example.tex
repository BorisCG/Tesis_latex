...
\begin{align}
	\frac{\partial \rho}{\partial t} + \nabla \cdot (\rho \vec{v}) =& 0 \label{eq:3_32} \\
	\frac{\partial \vec{v}}{\partial t} + (\vec{v} \cdot \nabla) \vec{v} =& - \frac{\nabla p}{\rho} \label{eq:3_33} \\
	\frac{\partial u}{\partial t} + (\vec{v} \cdot \nabla) u =& - \frac{p}{\rho} (\nabla \cdot \vec{v}) \label{eq:3_34}
\end{align}
...
\begin{equation}
	L_{SPH} = \sum_{i=1}^{N} m_i \left[\frac{1}{2} v_i^2 - u(\rho_i)\right] \label{eq:3_lag_discrete}
\end{equation}

where $m_i$, $v_i$, $\rho_i$ are the mass, velocity and density of each particle, which represent a fluid element. In order to use (\ref{eq:3_lag_discrete}) as the Lagrangian of the system, is necessary to use a suitable expression for the density.

In SPH, the density at position $\mathbf{r}$ is calculated as an average over all particles:

\begin{equation}
	\rho (\mathbf{r}) = \sum_{i} m_i W(\mathbf{r} - \mathbf{r}_i; h) \label{eq:3_dens_SPH}
\end{equation}

where $W(\mathbf{r} - \mathbf{r}_i; h)$ is a smoothing kernel of width h. The kernel has to fulfill two conditions: his integral over space should be unity and that in the limit $h \to 0$ approach to Dirac delta function. Note that with the first constrain, the integration of (\ref{eq:3_dens_SPH}) gives the total mass of the system.
