...
This thesis investigates the role of gas in the formation of supermassive black hole (SMBH) seeds via stellar collisions in dense clusters. In contrast with other works, we do not take into account gas accretion by stars, since we are interested exclusively in the gravitational interaction between gas and stars.
...
The gas dynamics are solved using the Smoothed Particle Hydrodynamics method \citep{gingold1977smoothed, lucy1977numerical}. In SPH, hydrodynamic equations are solved from a Lagrangian approach, sampling the gas by a discrete set of points, instead of the classical continuous. Consider a gas described by density $\rho$, pressure $p$ and specific internal energy $u$. For an inviscid fluid, the Lagrangian take the form:

\begin{equation}
	L = \int \left\{ \frac{1}{2} v^2 - u[\rho(\mathbf{r})] \right\} \rho d^3\mathbf{x} \label{eq:3_Lag_clasic}
\end{equation}

where the specific internal energy is related to the pressure by $\partial u / \partial \rho = p/\rho^2$, and the position vector is $\mathbf{r} = (\mathbf{x}_1, \mathbf{x}_2, \mathbf{x}_3)$. The Euler equations of motion can be derived from a variational principle in the Lagrangian \citep{rasio2000particle}:

\begin{align}
	\frac{\partial \rho}{\partial t} + \nabla \cdot (\rho \vec{\mathbf{v}}) =& 0 \label{eq:3_32} \\
	\frac{\partial \vec{\mathbf{v}}}{\partial t} + (\vec{\mathbf{v}} \cdot \nabla) \vec{\mathbf{v}} =& - \frac{\nabla p}{\rho} \label{eq:3_33} \\
	\frac{\partial u}{\partial t} + (\vec{\mathbf{v}} \cdot \nabla) u =& - \frac{p}{\rho} (\nabla \cdot \vec{\mathbf{v}}) \label{eq:3_34}
\end{align}

where (\ref{eq:3_32}) is the equation of continuity, (\ref{eq:3_33}) is the equation of motion and (\ref{eq:3_34}) is the internal energy equation. In SPH, instead of (\ref{eq:3_Lag_clasic}), a discrete Lagrangian is used:

\begin{equation}
	L_{SPH} = \sum_{i=1}^{N} m_i \left[\frac{1}{2} \mathbf{v}_i^2 - u(\rho_i)\right] \label{eq:3_lag_discrete}
\end{equation}

where $m_i$, $v_i$, $\rho_i$ are the mass, velocity and density of each particle, which represent a fluid element. $u(\rho_i) = u_i$ is the specific internal energy. In order to use (\ref{eq:3_lag_discrete}) as the Lagrangian of the system, is necessary to use a suitable expression for the density.

In SPH, the density at position $\mathbf{r}$ is calculated as an average over all particles:

\begin{equation}
	\rho (\mathbf{r}) = \sum_{j} m_j W(\mathbf{r} - \mathbf{r}_j; h) \label{eq:3_dens_SPH}
\end{equation}

where $W(\mathbf{r} - \mathbf{r}_i; h)$ is a smoothing kernel of width h (smoothing length). The kernel has to fulfill two conditions: his integral over space should be unity and that in the limit $h \to 0$ approach to Dirac delta function. Note that with the first constrain, the integration of (\ref{eq:3_dens_SPH}) gives the total mass of the system.

In the case of FI, the cubic spline kernel is used \citep{pelupessy2005numerical,monaghan1992smoothed}:
...
The last ingredient before deriving the equations of the gas is the smoothing parameter $h$. The dynamic range is controlled for the smoothing parameter. In actual codes is common to use an adaptive parameter $h_i$ for each particle, which adapts for the local density of particles, being common to set $h_i$ such that the number of neighbors for each particle $i$ is constant. The use of adaptive $h_i$ introduce errors in conservation of energy and entropy since $\nabla h_i$ terms are introduced. Including this extra terms can be cumbersome \citep{nelson1994variable}. Another approach is to consider $h_i$ as variables in the Lagrangian \citep{springel2002cosmological}. The conditions over $h_i$:

\begin{equation}
	\frac{4 \pi}{3} h_i^3(\rho_i + \tilde{\rho}) = M_{SPH}
\end{equation}

can be included as constrains through Lagrangian multipliers. Here $M_{SPH} = \bar{m} N_{nb}$, $\bar{m}$ is the mean mass of SPH particles and $N_{nb}$ is the number of neighbors. The extra term $\tilde{\rho}$ is to avoid numerical problems ($h_i \to \infty$ as $\rho_i \to 0$) \citep{pelupessy2005numerical}.

With the notation $W_{ij}(h_i) = W(|r_i - r_j|, h_i)$ and considering the symmetrized kernel $\bar{W}_{ij} = [W_{ij}(h_i) + W_{ij}(h_j)]/2$, the equation of motion is:
...
The artificial viscosity (AV) $\Pi_{ij}$ is necessary to smooth out discontinuities. Entropy is only generated in shocks through this artificial viscosity. The expression for the AV used in Fi \citep{pelupessy2005numerical,monaghan1992smoothed} is:
...
with $\mathbf{v}_{ij} = \mathbf{v}_i - \mathbf{v}_j$ and $\mathbf{r}_{ij} = \mathbf{r}_i - \mathbf{r}_j$. The constants $\alpha$ and $\beta$ are of order one (with default values $\alpha = 0.5$, $\beta = 1$), and the parameter $\eta$ is to prevent numerical divergences. The quantities overlaid with a bar are symmetrized. In particular, $\bar{c}_{ij} = (c_i + c_j)/2$ is the average sound speed between particles $i$ and $j$. The conditions in the cases of eq~\ref{eq:3.45} ensures that the AV is only activated when particles $i$ and $j$ are getting closer. Moreover, as $\mu_{ij}$ is always negative, then $\Pi_{ij} \geq 0$. As is shown in eq (apendice por hacer), the term $-\nabla_i W_{ij}(h_i) \propto (\mathbf{r}_i - \mathbf{r}_i)$ implies that the AV only decrease the kinetic energy. By conservation, thermal energy is increased.

Finally, is necessary the evolution of the thermal state of the system. FI has the possibility of evolve the entropic function or internal energy. Considering an adiabatic evolution of gas, the pressure for particle $i$ is:

\begin{equation}
	P_i = A_i \rho_i ^{\gamma}
\end{equation} 

with $A_i$ the entropic function, $\rho_i = \rho (\mathbf{r}_i)$ and adiabatic index $\gamma$. The internal energy is related to the entropic function and pressure:
...
The entropic function is evolved by
...
where $\Gamma$ and $\Lambda$ are external heating and cooling terms. On the other hand, the system can evolve following the internal energy equation:
