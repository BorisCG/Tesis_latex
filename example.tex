...
The proposed scenarios presented in (referencia a capitulo) require high numerical precision to accurately model close encounters and direct collisions. We therefore employ the \texttt{ph4} N-body code from the AMUSE framework \cite{zwart2018astrophysical}.
...
The gas dynamics are modelled using the \texttt{Fi} code, an MPI-parallel smoothed particle hydrodynamics (SPH) implementation within the AMUSE framework. We focus on the aspects most relevant to our simulations; a comprehensive description of the code can be found in \cite{pelupessy2005numerical}.
...
\texttt{Fi} employs the Barnes-Hut (BH) tree algorithm \cite{barnes1986hierarchical}. This method uses a hierarchical oct-tree spatial decomposition and a geometric opening criterion to approximate gravitational forces.
